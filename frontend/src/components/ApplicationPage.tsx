...
export default function ApplicationPage({ courses, onCancel }: ApplicationPageProps) {
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 3) {
      if (!requiredDocuments.every((type) => form.documents.some((doc) => doc.documentType === type && doc.fileUrl.trim()))) {
        showError('Application Error', 'Please provide all required documents.');
        return false;
      }
      if (!form.documents.every((doc) => !doc.fileUrl || isValidUrl(doc.fileUrl))) {
        showError('Application Error', 'Enter valid document URLs.');
        return false;
      }
      return true;
    }
                  ...
                  <div className="space-y-4">
                    {form.documents.map((doc, index) => (
                      <div key={doc.id} className="rounded-3xl border border-slate-200 bg-white p-4">
                        ...
                        <input value={doc.fileUrl} onChange={(e) => handleDocumentChange(index, 'fileUrl', e.target.value)} placeholder="Paste a document URL" className="mt-3 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-blue-500" />
                        <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
                          <span>{doc.fileName ? `Selected: ${doc.fileName}` : 'No file entered yet'}</span>
                          <span>{doc.fileUrl ? 'Ready for review' : 'Required'}</span>
                        </div>
                      </div>
                    ))}
                    <p className="text-xs text-slate-500">Use a secure public link for each document. Document links are reviewed during application processing.</p>
                  </div>
                )}
                      ...
                      <div className="grid gap-3">
                        {form.documents.map((doc) => (
                          <div key={doc.id} className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                            ...
                            <p className="mt-2 text-sm text-slate-700 break-all">{doc.fileUrl || 'No document link provided'}</p>
                          </div>
                        ))}
                      </div>
                    </div>
