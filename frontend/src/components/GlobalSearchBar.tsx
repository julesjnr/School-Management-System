  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-xl" id="global-search-container">
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search students, courses, inventory..."
          value={searchTerm}
          onChange={(e) => {
          ...
          className="w-full bg-slate-100/80 dark:bg-slate-800/80 text-slate-900 dark:text-slate-100 placeholder-slate-400 text-xs rounded-2xl pl-10 pr-10 py-2.5 border border-transparent hover:border-slate-200 focus:bg-white dark:focus:bg-slate-900 focus:border-[#2563EB] focus:ring-2 focus:ring-blue-500/20 transition-all duration-200 outline-none"
        />
        {searchTerm && (
          <button
            type="button"
      ...
      {isOpen && searchTerm && (
        <div className="absolute left-0 right-0 mt-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-xl z-50 overflow-hidden font-sans max-h-[460px] flex flex-col animate-in fade-in duration-150">
          {/* Header */}
          <div className="px-4 py-3 bg-slate-50/80 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center text-[10px] text-slate-400 uppercase tracking-wider font-semibold">
            <span>Search Results</span>
            <span className="bg-blue-50 text-[#2563EB] px-2 py-0.5 rounded-full font-bold">{totalMatches} match{totalMatches === 1 ? '' : 'es'}</span>
          </div>
