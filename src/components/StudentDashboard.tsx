...
export default function StudentDashboard({
  ...
  return (
    <div className="bg-slate-55 min-h-screen p-4 sm:p-6 lg:p-8 flex flex-col space-y-6 animate-fade-in" id="student-dashboard-root">
      ...
      {/* HEADER BAR */}
      <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-xs" id="student-header-row">
        <div className="flex items-center gap-4">
          <div className="relative group/avatar cursor-pointer select-none shrink-0" onClick={() => setIsCameraModalOpen(true)}>
            {student.avatar ? (
              <img 
                src={student.avatar} 
                alt={student.name} 
              ...
              <div className="w-14 h-14 rounded-xl bg-blue-100 text-blue-700 flex items-center justify-center font-black text-xl border border-blue-200">
                {student.name.charAt(0)}
              </div>
            )}
            <div className="absolute inset-0 bg-slate-900/60 text-white rounded-xl flex flex-col items-center justify-center opacity-0 group-hover/avatar:opacity-100 transition-opacity duration-200">
              <Camera className="w-4 h-4 text-white" />
              <span className="text-[7.5px] font-black uppercase mt-0.5 tracking-wider">Update</span>
            </div>
          </div>
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 text-[9.5px] bg-slate-100 hover:bg-slate-200 text-slate-755 px-2.5 py-0.5 rounded-full border border-slate-200 font-extrabold cursor-pointer transition-colors"
                >
            ...
            <p className="text-xs text-slate-500 font-mono">Admission No: {student.admissionNo} • {student.cohort}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto self-stretch md:self-auto">
          <div className="text-left md:text-right hidden sm:block">
            ...
            <span className={`text-base font-bold block ${outstandingBal > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
          ...
          <button
            ...
            className="w-full md:w-auto bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-semibold px-4 py-2.5 rounded-xl transition-colors cursor-pointer"
          >
            View Website
          </button>
          <button
            type="button"
            onClick={onLogout}
            className="w-full md:w-auto bg-slate-100 hover:bg-slate-205 text-slate-700 text-xs font-semibold px-4 py-2.5 rounded-xl transition-colors cursor-pointer"
          >
            Sign Out Portal
          </button>
        </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-2xs flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold block">Enrolled Units</span>
            <span className="text-lg font-extrabold text-slate-800">{student.enrolledUnits.length} Modules</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-2xs flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold block">Average CAT + Exam</span>
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-2xs flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold block">Fees Settled Status</span>
            <span className="text-lg font-extrabold text-slate-800">
              {outstandingBal === 0 ? 'Fully Paid' : `${Math.round((totalPaid / totalInvoiced) * 100)}% Settled`}
            </span>
          </div>
        </div>
      </div>

      {/* DEGREE PROGRESS ROADMAP MONITOR - Hide globally when on dashboard overview tab */}
      {activeTab !== 'dashboard' && (
        <DegreeProgress 
          student={student} 
      ...
      <div className="flex flex-wrap md:flex-nowrap bg-white rounded-xl border border-slate-100 p-1 text-[11px] font-bold gap-1 mt-2">
        <button
          type="button"
          onClick={() => setActiveTab('dashboard')}
          className={`flex-1 py-3 text-center rounded-lg uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'dashboard' ? 'bg-blue-650 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          My Dashboard
        </button>
        <button
          ...
          className={`flex-1 py-3 text-center rounded-lg uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'grades' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-3 text-center rounded-lg uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'financials' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          My Student Financials
        </button>
        <button
          type="button"
          onClick={() => setActiveTab('materials')}
          className={`flex-1 py-3 text-center rounded-lg uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'materials' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          Supplementary Materials ({mockStudyMaterials.filter(m => student.enrolledUnits.includes(m.unit)).length})
        </button>
        <button
          type="button"
          onClick={() => setActiveTab('units')}
          className={`flex-1 py-3 text-center rounded-lg uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'units' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          Register / Drop Units
        </button>
        <button
          ...
          className={`flex-1 py-3 text-center rounded-lg uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'officeHours' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          id="btn-student-library-tab"
        >
          📖 E-Library Catalog (OPAC)
        </button>
      </div>
